...
class Radio:
    ...
    def toggle_on_off(self):
        if not self.power:
            ...
            if self.current_channel.get_channel_type() == "STREAM":
                ...
                self.play()
            elif self.current_channel.get_channel_type() == "BLUETOOTH":
    ...
    def set_volume(self, volume: int) -> int:
        ...
        if self.current_channel.get_channel_type() == "STREAM":
            ...
            self.volume = self.current_channel.set_volume(volume)
        else:
            self.volume = -1
    ...
    def volume_up(self) -> int:
        if self.power:
            return self.set_volume(self.volume + self.__volume_step)
        else:
            return -1
    ...
    def mute(self) -> int:
        if self.power:
            ...
            if self.current_channel.get_channel_type() == "STREAM":
                self.media_player.audio_set_mute(self.is_mute)
            elif self.current_channel.get_channel_type() == "BLUETOOTH":
                self.current_channel.set_volume(0 if self.is_mute else self.volume)
            return 0 if self.is_mute else self.volume
        else:
            return -1
